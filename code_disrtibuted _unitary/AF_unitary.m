...
data1=zeros(T,N/3);
data2=zeros(T,N/3);% Received data matrix
A1= eye(L);
...
data_BS1=zeros(T,N/3);
for c = 1:3:N-2
   % for j=1:T
      data_BS1(:,(c-1)/3+1) = map_3bits_to_unitary(data_BS10(c:c+2),unitary_array);
    %end
end
...
%% Channel gains
        channel_BS1_RS1=(1/sqrt(2)).*[randn(1,N/3)+1i*randn(1,N/3)];% Channel coefficients for Source-Relay1 Link
        a11=sum((abs(channel_BS1_RS1)))/M; %Average channel gain for Soure-Relay Link Channel
                
        channel_RS1_MS1=(1/sqrt(2)).*[randn(1,N/3)+1i*randn(1,N/3)];% Channel coefficients for Relay1-Desination Link
        b11=sum((abs(channel_RS1_MS1)))/M; %Average channel gain for Relay-Desination Link Channel
                
        channel_BS1_RS2=(1/sqrt(2))*[randn(1,N/3)+1i*randn(1,N/3)];% Channel coefficients for Source-Relay2 Link
        a12=sum((abs(channel_BS1_RS2)))/M; %Average channel gain for Soure-Desination Link Channel
        
        channel_RS2_MS1=(1/sqrt(2)).*[randn(1,N/3)+1i*randn(1,N/3)];% Channel coefficients for Relay2-Desination Link
        b21=sum((abs(channel_RS2_MS1)))/M; %Average channel gain for Relay-Desination Link Channel
        
 %% AWGN     
        nbr1=(1/sqrt(2))*(randn(T,N/3)+1i*randn(T,N/3));%AWGN for at Relay for Soure-Relay1 Link
        nr1m=(1/sqrt(2))*(randn(T,N/3)+1i*randn(T,N/3));%AWGN for at Desination  for Relay1-Desination Link
        nbr2=(1/sqrt(2))*(randn(T,N/3)+1i*randn(T,N/3));%AWGN for at Desination  for source-relay2 Link
        nr2m=(1/sqrt(2))*(randn(T,N/3)+1i*randn(T,N/3));%AWGN for at Desination  for relay2-Desination Link
...
for       i=1:length(snr_db)
   ...
   %% AMPLIFICATION FACTOR
               Beta1=sqrt(Pr1./(Pb+1));
               Beta2=sqrt(Pr2./(Pb+1));
               data_BS1_RS1= zeros(8,N/3);
               data_RS1_MS1= zeros(8,N/3);
               data_BS1_RS2= zeros(8,N/3);
               %data_MS1_RS2= zeros(8,N/3);
               data_RS2_MS1= zeros(8,N/3);
  ...
  %% RECEIVED DATA AT RELAY2 FROM BS         
    ...
    %% DEMODULATION NON COHERENT SUB OPTIMAL DETECTOR
            for k=1:N/3
                data1(:,k) = demod_unitary_noncoherent(data_MS(:,k),unitary_array,T);
                %% CALCULATING ERRORS
    ...
    %% DEMODULATION PARTIALLY COHERENT  OPTIMAL DETECTOR
            for l=1:N/3
                C = diag([((snr_linear(i)*(abs(channel_RS1_MS1(l))^2))/(1+snr_linear(i)*(abs(channel_RS1_MS1(l))^2)));((snr_linear(i)*(abs(channel_RS2_MS1(l))^2))/(1+snr_linear(i)*(abs(channel_RS2_MS1(l))^2)))]);
                data2(:,l) = demod_unitary_partial_coherent(data_MS(:,l),unitary_array,C,T);
                %[error_af2(i),~]=size(find((data_BS1(:,k)- data2(:,k))));
            if abs(data_BS1(:,l)- data2(:,l)) == 0
                  error_af2(i)= error_af2(i);
            else
                error_af2(i)= error_af2(i)+ 1;
           end
             
            end
       
        %error_af2(i)=size(col2,2);
   %%   MRC AT RECEIVER 
       % data_rd=data_RS1_MS1.*conj(channel_BS1_RS1.*channel_RS1_MS1)+ data_RS2_MS1.*conj(channel_BS1_RS2.*channel_RS2_MS1);
...
save('Af_non_L=8.mat','error_af1');
save('Af_partial_L=8.mat','error_af2');

...
%% Channel gains
        Bs_Rs=sqrt(K/K+1)*ones(2,M)+sqrt(1/K+1).*((1/sqrt(2)).*[randn(1,M)+1i*randn(1,M);randn(1,M)+1i*randn(1,M)]);% Channel coefficients for Source-Relay1 Link
        Rs_Ms=sqrt(K/K+1)*ones(2,M)+sqrt(1/K+1).*((1/sqrt(2)).*[randn(1,M)+1i*randn(1,M);randn(1,M)+1i*randn(1,M)]);
...
save('DF_rice_two_diff.mat','error_af1');

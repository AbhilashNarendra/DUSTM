...
for       i=1:length(snr_db)
               ...
               % No(i)=Pt./snr_linear(i);            % Noise power
                Pb = 3*snr_linear(i)/8 + sqrt(9*snr_linear(i)*snr_linear(i) + 24*snr_linear(i))/8;
  ...
  %% RECEIVED DATA AT RELAY2 FROM BS         
    ...
    %% DEMODULATION PARTIALLY COHERENT  OPTIMAL DETECTOR
            for l=1:M
                C = diag([((snr_linear(i)*(abs(channel_RS1_MS1(l))^2))/(1+snr_linear(i)*(abs(channel_RS1_MS1(l))^2)));((snr_linear(i)*(abs(channel_RS2_MS1(l))^2))/(1+snr_linear(i)*(abs(channel_RS2_MS1(l))^2)));((snr_linear(i)*(abs(channel_RS3_MS1(l))^2))/(1+snr_linear(i)*(abs(channel_RS3_MS1(l))^2)))]);
                data2(:,l) = demod_unitary_partial_coherent_3(data_MS(:,l),unitary_array,C,T);
...
save('Af_rayleigh3_non.mat','error_af1');
save('Af_rayleigh3_partial.mat','error_af2');

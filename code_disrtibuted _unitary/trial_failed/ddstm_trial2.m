...
unitary_array =[u1 u2 u3 u4 u5 u6 u7 u8];
       ...
       data_BS1= zeros(2,M);
        ...
        for f=1:3:N-2
          data_BS1(:,(f-1)/3+1) = modulate_diff_1(data_BS10(:,f:f+2),unitary_array); 
        end
...
for       i=1:length(snr_db)
  ...
  %% RECEIVED DATA AT RELAY2 FROM BS         
    ...
    %% RECEIVED DATA AT MS FROM RS2     
               data_RS2_MS1(j,:) = Beta2.*A2(j,j).*data_BS1_RS2(j,:).*channel_RS2_MS1 ;
    ...
    %% DEMODULATION NON COHERENT SUB OPTIMAL DETECTOR
            for k=1:M-1
                ...
                data1(:,k+1) = ddstm_demod1_1(data_MS(:,k),data_MS(:,k+1),unitary_array,L);
            ...
            %data10 = zeros(1,N);
             ...
             for v=1:length(data1)-1  
                 ...
                 data10(3*v+1:1:3*v+3) = [1 1 0];
                  elseif data1(v+1)== 7
                 data10(3*v+1:1:3*v+3) = [1 1 1];
                 
                 end
             end
             
           
             error_af1(i)= size(find(data10(4:length(data10))-data_BS10(4:length(data10))),2);       
